...
const normalizeStats = (data: any): DashboardStats => ({
  clients: safeNumber(data?.clients ?? data?.nb_clients),
  devis: safeNumber(data?.devis ?? data?.nb_devis),
  ordres: safeNumber(data?.ordres ?? data?.nb_ordres),
  factures: safeNumber(data?.factures ?? data?.nb_factures),
  paiements: safeNumber(data?.paiements ?? data?.nb_paiements),
  caisse: safeNumber(data?.solde_caisse ?? data?.caisse),
  banque: safeNumber(data?.solde_banque ?? data?.banque),
  creances: safeNumber(data?.creances ?? data?.total_creances),
  ca_mensuel: safeNumber(data?.ca_mensuel ?? data?.ca_mois),
  ca_annuel: safeNumber(data?.ca_annuel ?? data?.ca_total),
});

const normalizeGraphiques = (data: any): DashboardGraphiques => ({
  ca_mensuel: Array.isArray(data?.ca_mensuel) 
    ? data.ca_mensuel.map((m: any) => ({
        mois: String(m?.mois ?? m?.label ?? ''),
  ...
  top_clients: Array.isArray(data?.top_clients)
    ? data.top_clients.map((c: any) => ({
        ...
        montant: safeNumber(c?.montant ?? c?.ca_total ?? c?.total),
      }))
    : [],
  factures_par_categorie: Array.isArray(data?.factures_par_categorie)
    ? data.factures_par_categorie.map((f: any) => ({
