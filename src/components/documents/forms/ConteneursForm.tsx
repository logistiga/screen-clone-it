...
interface ConteneursFormProps {
  armateurs: Partenaire[];
  transitaires: Partenaire[];
  representants: Partenaire[];
  onDataChange: (data: ConteneursFormData) => void;
  initialData?: Partial<ConteneursFormData>;
}

export default function ConteneursForm({
  armateurs,
  transitaires,
  representants,
  onDataChange,
  ...
  const [primeTransitaire, setPrimeTransitaire] = useState<number>(initialData?.primeTransitaire || 0);
  const [primeRepresentant, setPrimeRepresentant] = useState<number>(initialData?.primeRepresentant || 0);
  const [conteneurs, setConteneurs] = useState<LigneConteneur[]>(initialData?.conteneurs || [getInitialConteneur()]);
  ...
  const handleAddConteneur = () => {
    const newConteneurs = [...conteneurs, getInitialConteneur()];
  ...
  const handleAddOperation = (conteneurId: string) => {
  ...
  const handleRemoveOperation = (conteneurId: string, operationId: string) => {
    const newConteneurs = conteneurs.map((c) =>
      c.id === conteneurId ? { ...c, operations: c.operations.filter((op) => op.id !== operationId) } : c
    );
    setConteneurs(newConteneurs);
    updateParent(newConteneurs);
  };

  const handleOperationChange = (conteneurId: string, operationId: string, field: string, value: string | number) => {
    const newConteneurs = conteneurs.map((c) => {
      if (c.id === conteneurId) {
        return {
          ...c,
                ...
                <div className="space-y-3 pl-4 border-l-2 border-muted">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => handleAddOperation(conteneur.id)}
                  ...
                  {conteneur.operations.length === 0 ? (
                    <p className="text-sm text-muted-foreground italic">Aucune opération</p>
                  ) : (
                        ...
                        <div key={op.id} className="p-3 border rounded-lg bg-muted/30 space-y-3">
                          <div className="flex items-center justify-end">
                            <Button
                              ...
                              variant="ghost"
                              size="icon"
                              onClick={() => handleRemoveOperation(conteneur.id, op.id)}
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            <div className="space-y-1 col-span-2">
                              ...
                              <Select
                                value={op.type}
                                onValueChange={(value) => handleOperationChange(conteneur.id, op.id, "type", value)}
                              >
                            ...
                            <div className="space-y-1">
                              ...
                              <Input
                                ...
                                value={op.quantite}
                                onChange={(e) =>
                                  handleOperationChange(conteneur.id, op.id, "quantite", parseInt(e.target.value) || 0)
                                }
                              />
                            </div>
                            <div className="space-y-1">
                              ...
                              <Input
                                ...
                                onChange={(e) =>
                                  handleOperationChange(conteneur.id, op.id, "prixUnitaire", parseInt(e.target.value) || 0)
                                }
                              />
                            </div>
                          </div>
