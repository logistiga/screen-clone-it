...
import { 
  ...
  Calendar, Hash, Warehouse, RotateCcw, MapPin, Receipt 
} from "lucide-react";
...
export function FacturePreview({
  ...
  const getCategorieConfig = () => {
    switch (categorie) {
      ...
      default:
        return { icon: <Receipt className="h-5 w-5" />, color: "bg-muted" };
    }
  };
          ...
          {dateEcheance && (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <span>{new Date(dateEcheance).toLocaleDateString('fr-FR')}</span>
              </div>
            </motion.div>
          )}
