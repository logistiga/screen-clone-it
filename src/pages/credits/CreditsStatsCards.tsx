...
import { formatMontantCompact, toSafeNumber } from "./useCreditsData";
    ...
    <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
      {loadingStats ? (
        ...
        <>
          ...
          <DocumentStatCard title="Total remboursé" value={formatMontantCompact(stats?.total_rembourse || 0)} icon={TrendingDown} subtitle={`${toSafeNumber(stats?.taux_remboursement_global).toFixed(1)}% du total`} variant="success" delay={0.1} />
