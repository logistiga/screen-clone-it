...
class PrimeController extends Controller
{
    ...
    /**
     * Décaisser une prime (créer sortie de caisse depuis comptabilité)
     */
    public function decaisser(Request $request, Prime $prime): JsonResponse
    {
        // Vérifier que la prime est bien payée (en attente de décaissement)
        if (!in_array($prime->statut, ['Payée', 'Partiellement payée'])) {
            return response()->json([
                'message' => 'Cette prime n\'est pas en attente de décaissement'
            ], 422);
        }
        ...
        try {
            ...
            $prime->load(['transitaire', 'representant', 'facture']);

            // Déterminer le bénéficiaire
            $beneficiaire = $prime->transitaire 
                ? $prime->transitaire->nom 
                : ($prime->representant ? "{$prime->representant->nom} {$prime->representant->prenom}" : 'Partenaire');
            ...
            $mouvement = MouvementCaisse::create([
                ...
                'description' => "Décaissement prime - {$beneficiaire}" . ($prime->facture ? " (Facture: {$prime->facture->numero})" : ''),
                'beneficiaire' => $beneficiaire,
                'reference' => $request->reference,
                'mode_paiement' => $request->mode_paiement,
                'date' => now()->toDateString(),
            ...
            // Mettre à jour le statut de la prime à "Décaissée"
            $prime->update([
                'statut' => 'Décaissée',
                'date_paiement' => now(),
            ]);

            Audit::log('create', 'decaissement_prime', "Décaissement prime: {$prime->montant} - {$beneficiaire}", $mouvement->id);
