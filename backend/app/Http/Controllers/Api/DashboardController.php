...
class DashboardController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            $cacheKey = CacheService::key(CacheService::PREFIX_DASHBOARD . 'index', [
                'date_debut' => $dateDebut,
                'date_fin' => $dateFin,
            ]);

            $stats = CacheService::remember($cacheKey, CacheService::TTL_DASHBOARD, function () use ($dateDebut, $dateFin) {
                return [
                    // Compteurs généraux
                    'clients' => [
                        'total' => Client::count(),
    ...
    public function graphiques(Request $request): JsonResponse
    {
        try {
            ...
            $cacheKey = CacheService::key(CacheService::PREFIX_DASHBOARD . 'graphiques', [
                'annee' => $annee,
            ]);

            $graphiques = CacheService::remember($cacheKey, CacheService::TTL_GRAPHIQUES, function () use ($annee) {
                return [
    ...
    {
        try {
            $cacheKey = CacheService::PREFIX_DASHBOARD . 'alertes';

            $alertes = CacheService::remember($cacheKey, CacheService::TTL_ALERTES, function () {
                return [
                    // Factures en retard
    ...
    {
        try {
            $cacheKey = CacheService::PREFIX_DASHBOARD . 'activite';

            $activites = CacheService::remember($cacheKey, CacheService::TTL_ACTIVITE, function () {
                // Dernières factures
    ...
    /**
     * Invalider manuellement le cache du dashboard (admin)
     */
